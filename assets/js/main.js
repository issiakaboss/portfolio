...
//   if (headerToggleBtn) { // Ajoute cette condition
...
/**
* Template Name: iPortfolio - Version Corrigée
* Author: BootstrapMade.com
* Updated: 2026 par Gemini
*/
(function() {
  "use strict";

  /**
   * Helper function : Sélectionne un élément plus facilement
   */
  ...
  /**
   * Fermer le menu mobile lors d'un clic sur un lien (même page)
   */
  document.addEventListener('click', function(e) {
    const navLink = e.target.closest('#navmenu a');
    if (navLink && header && header.classList.contains('header-show')) {
      headerToggle();
    }
  });
  ...
  select('.isotope-layout', true).forEach(function(isotopeItem) {
    ...
    if (!container) return; // Sécurité si on est sur la page détails
  ...
  function initSwiper() {
    select(".init-swiper", true).forEach(function(swiperElement) {
      ...
      // Gestion de la boucle Swiper si peu de slides
      if (config.loop && swiperElement.querySelectorAll('.swiper-slide').length < 2) {
        config.loop = false;
      }
  ...
  /**
   * Correction du scroll au chargement (Hash links)
   */
  window.addEventListener('load', function(e) {
    if (window.location.hash) {
